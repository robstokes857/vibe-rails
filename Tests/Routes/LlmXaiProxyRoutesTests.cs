...
public sealed class LlmXaiProxyRoutesTests
{
    ...
    public async Task AuthenticatedRequest_RewritesAndRelaysWithoutLeakingLocalCredentials()
    {
        ...
        Assert.Equal(
            "https://api.x.ai/v1/chat/completions?api-version=test",
        ...
        Assert.DoesNotContain(
            ...
            key => key.Equals(LlmProxyXaiConfig.SessionHeaderName, StringComparison.OrdinalIgnoreCase));
        Assert.DoesNotContain(
            result.Upstream.Headers.Keys,
            key => key.Equals(LlmProxyXaiConfig.TabHeaderName, StringComparison.OrdinalIgnoreCase));
        Assert.DoesNotContain(
            result.Upstream.Headers.Keys,
        ...
        Assert.Equal("xai", savings.Provider);
        ...
        Assert.Equal("https://api.x.ai/v1/chat/completions", activity.Target);
    ...
    public async Task TokenSaverDisabled_StillRecordsTheExchange()
    {
        ...
        Assert.Equal("xai", exchange.Provider);
        Assert.Equal("/llm/xai/v1/chat/completions", exchange.Path);
        ...
        try
        {
            ...
            using var request = new HttpRequestMessage(
                ...
                new Uri(new Uri(app.Urls.First()),
                    "/llm/xai/v1/chat/completions?api-version=test"));
            if (authenticated)
            {
                request.Headers.TryAddWithoutValidation(LlmProxyXaiConfig.SessionHeaderName, SessionToken);
                request.Headers.TryAddWithoutValidation(LlmProxyXaiConfig.TabHeaderName, TabToken);
            }
