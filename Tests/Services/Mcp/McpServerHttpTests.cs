...
public class McpServerHttpTests : IAsyncLifetime
{
    ...
    [Theory]
    [InlineData("python_http_smoke")]
    [InlineData("python_script_signing_help")]
    public async Task CallPythonTool_IsUnavailable(string toolName)
    {
        await using var client = await ConnectAsync(TestContext.Current.CancellationToken);
        var error = await Assert.ThrowsAsync<ModelContextProtocol.McpProtocolException>(() => client.CallToolAsync(
            toolName, new Dictionary<string, object?>(), TestContext.Current.CancellationToken));
