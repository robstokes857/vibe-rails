...
public sealed class LlmParser : ILlmParser
{
    ...
    // "glm-5.2" / "grok-4.6" / "glm-5.3" can't round-trip through Enum.TryParse. Map them
    // explicitly here. Glm52 / Glm53 launch `opencode` with a pinned --model flag;
    // Grok46 launches the native `grok` binary (see CommandService.PrepareSession).
    ...
    // "glm-5.2" / "grok-4.6" / "glm-5.3" for the hyphenated wire names, the canonical enum name otherwise. Every outbound
