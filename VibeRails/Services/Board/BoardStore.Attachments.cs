...
public sealed partial class BoardStore
{
    static partial void EnsureAttachmentSchema(SqliteConnection connection, SqliteTransaction transaction)
    {
        ...
        using (var columns = connection.CreateCommand())
        {
            ...
            columns.CommandText = "PRAGMA table_info(BoardAttachments);";
            using var reader = columns.ExecuteReader();
            while (reader.Read())
                if (reader.GetString(1) == "DeletedUTC") hasDeleted = true;
        }
    ...
    private static async Task ValidateAttachmentCountAsync(SqliteConnection connection, SqliteTransaction transaction, string cardId, CancellationToken cancellationToken)
    {
        ...
        if (Convert.ToInt64(await command.ExecuteScalarAsync(cancellationToken)) >= BoardService.MaxAttachmentsPerCard)
            throw new BoardValidationException($"A card can hold at most {BoardService.MaxAttachmentsPerCard} current attachments.");
    }
    ...
    public async Task<BoardAttachmentContent?> GetAttachmentContentAsync(string projectPath, string idOrKey, string attachmentId, CancellationToken cancellationToken = default)
    {
        ...
        var bytes = reader.IsDBNull(7) ? BoardService.DecodeAttachmentDataUrl(record.DataUrl) : (byte[])reader.GetValue(7);
