    ...
    IJobScheduler scheduler) : IJobService
{
    ...
    public async Task<JobResponse> CreateJobAsync(CreateJobRequest request, CancellationToken cancellationToken = default)
    {
        if (request.EnvironmentId is null)
            throw JobServiceException.BadRequest("Choose an Environment / Worker before creating an automation Job.");
    ...
    public async Task<JobResponse> UpdateJobAsync(long id, UpdateJobRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (existing.EnvironmentId is not null && request.EnvironmentId is null)
            throw JobServiceException.BadRequest("A Job cannot be detached from its Environment / Worker. Choose another worker instead.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        // The Environment / Worker is the single source of truth for the CLI, model, args, and
        // initial message; the Job only owns when it runs and its timeout.
        LLM resolvedLlm;
        var resolvedPrompt = string.Empty;
        if (environmentId is null)
            throw JobServiceException.BadRequest("Choose an Environment / Worker.");
        ...
        if (!string.IsNullOrWhiteSpace(environment.CustomPrompt))
            ...
            throw JobServiceException.BadRequest("The selected environment / worker needs an Initial Message before it can run as a Job.");
