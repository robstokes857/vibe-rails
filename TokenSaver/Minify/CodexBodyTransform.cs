...
/// <summary>
/// Buffers qualifying Codex Responses API POST bodies and applies
/// <see cref="CodexResponsesRewriter"/>. As with the Anthropic transform, every rewrite failure
    ...
    ICompressionCaptureSink? captures = null) : ILlmProxyBodyTransform
{
        ...
        HttpRequest request, ILlmProxyEventSink events, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                events.Diagnostic(
                    "Codex proxy", "token-saver rewrite failed; forwarding original body.", ex);
    ...
    private static bool AppliesTo(HttpRequest request) =>
        ...
        && request.Path.Value?.EndsWith("/responses", StringComparison.OrdinalIgnoreCase) == true
