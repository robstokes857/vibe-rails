...
public sealed class BertSearchDbService : IBertSearchDbService
{
    ...
    public BertSearchDbService(string vectorDatabasePath, string stateDatabasePath)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(vectorDatabasePath);
    ...
    public string StateDatabasePath => _stateDatabasePath;

    public bool VectorDatabaseExists => File.Exists(_vectorDatabasePath);

    public int CountDocuments() => SqliteStorageErrors.Execute(() => CountDocumentsCore());
    public int CountSessions() => SqliteStorageErrors.Execute(() => CountSessionsCore());
    public int CountVectors() => SqliteStorageErrors.Execute(() => CountVectorsCore());
    public int CountSessionDocuments() => SqliteStorageErrors.Execute(() => CountSessionDocumentsCore());
    public int CountSessionVectors() => SqliteStorageErrors.Execute(() => CountSessionVectorsCore());
    ...
    public IReadOnlyList<BertStoredDocument> SearchByEmbedding(float[] embedding, int topK) => SqliteStorageErrors.Execute(() => SearchByEmbeddingCore(embedding, topK));
    public BertStoredDocument? GetSessionCapture(string documentId) => SqliteStorageErrors.Execute(() => GetSessionCaptureCore(documentId));
    public IReadOnlyList<BertStoredDocument> SearchSessionsByText(string query, int topK) => SqliteStorageErrors.Execute(() => SearchSessionsByTextCore(query, topK));
    public IReadOnlyList<BertStoredDocument> SearchSessionsByEmbedding(float[] embedding, int topK) => SqliteStorageErrors.Execute(() => SearchSessionsByEmbeddingCore(embedding, topK));
    public IReadOnlyDictionary<string, BertInputMetadata> GetMetadataByDocumentIds(IReadOnlyCollection<string> documentIds) => SqliteStorageErrors.Execute(() => GetMetadataByDocumentIdsCore(documentIds));
    public IReadOnlyList<BertFileChangeResponse> GetFileChanges(long userInputId) => SqliteStorageErrors.Execute(() => GetFileChangesCore(userInputId));
    public IReadOnlyList<BertFileChangeResponse> GetSessionFileChanges(string sessionId) => SqliteStorageErrors.Execute(() => GetSessionFileChangesCore(sessionId));
    public IReadOnlyDictionary<string, BertSessionMetadata> GetSessionMetadataByDocumentIds(IReadOnlyCollection<string> documentIds) => SqliteStorageErrors.Execute(() => GetSessionMetadataByDocumentIdsCore(documentIds));

    private int CountDocumentsCore()
    {
    ...
    private int CountSessionDocumentsCore()
    {
        if (!VectorDatabaseExists) return 0;
        using var connection = OpenVectorConnection(loadVectorExtension: false);
    ...
    private int CountSessionVectorsCore()
    {
        if (!VectorDatabaseExists) return 0;
        using var connection = OpenVectorConnection(loadVectorExtension: true);
    ...
    private string? GetLatestDocumentIdCore()
    {
    ...
    private IReadOnlyList<BertStoredDocument> GetCapturesCore(int skip, int take)
    {
    ...
    private IReadOnlyList<BertStoredDocument> GetCapturesBySessionIdCore(string sessionId)
    {
        if (!VectorDatabaseExists) return [];
        using var connection = OpenVectorConnection(loadVectorExtension: false);
        using var command = connection.CreateCommand();
    ...
    private IReadOnlyList<BertStoredDocument> GetSessionCapturesCore(int skip, int take)
    {
        if (!VectorDatabaseExists) return [];
        using var connection = OpenVectorConnection(loadVectorExtension: false);
    ...
    private BertStoredDocument? GetCaptureCore(string documentId)
    {
        if (!VectorDatabaseExists) return null;
        using var connection = OpenVectorConnection(loadVectorExtension: false);
        using var command = connection.CreateCommand();
    ...
    private IReadOnlyList<BertStoredDocument> SearchByTextCore(string query, int topK)
    {
    ...
    private IReadOnlyList<BertStoredDocument> SearchByEmbeddingCore(float[] embedding, int topK)
    {
        if (!VectorDatabaseExists) return [];
        using var connection = OpenVectorConnection(loadVectorExtension: true);
        using var command = connection.CreateCommand();
    ...
    private BertStoredDocument? GetSessionCaptureCore(string documentId)
    {
        if (!VectorDatabaseExists) return null;
        using var connection = OpenVectorConnection(loadVectorExtension: false);
    ...
    private IReadOnlyList<BertStoredDocument> SearchSessionsByTextCore(string query, int topK)
    {
        if (!VectorDatabaseExists) return [];
        using var connection = OpenVectorConnection(loadVectorExtension: false);
        if (!TableExists(connection, BertSearchSchema.SessionDocumentTableName)) return [];
    ...
    private IReadOnlyList<BertStoredDocument> SearchSessionsByEmbeddingCore(float[] embedding, int topK)
    {
        if (!VectorDatabaseExists) return [];
        using var connection = OpenVectorConnection(loadVectorExtension: true);
    ...
    private IReadOnlyDictionary<string, BertInputMetadata> GetMetadataByDocumentIdsCore(IReadOnlyCollection<string> documentIds)
    {
    ...
    private IReadOnlyList<BertFileChangeResponse> GetFileChangesCore(long userInputId)
    {
        if (!File.Exists(StateDatabasePath))
            return [];
    ...
    private IReadOnlyList<BertFileChangeResponse> GetSessionFileChangesCore(string sessionId)
    {
        if (!File.Exists(StateDatabasePath))
            return [];
    ...
    private IReadOnlyDictionary<string, BertSessionMetadata> GetSessionMetadataByDocumentIdsCore(IReadOnlyCollection<string> documentIds)
    {
    ...
    private SqliteConnection OpenVectorConnection(bool loadVectorExtension)
    {
        ...
        if (loadVectorExtension)
            return BertVectorDatabase.Open(_vectorDatabasePath, readOnly: true);
        return SqliteConnectionFactory.Open(
            new SqliteConnectionStringBuilder { DataSource = _vectorDatabasePath }.ToString(), readOnly: true);
    }

    private SqliteConnection OpenStateConnection()
    {
        return SqliteConnectionFactory.Open(
            new SqliteConnectionStringBuilder { DataSource = StateDatabasePath }.ToString(), readOnly: true);
    }
    ...
    /// <summary>
    /// Escapes the LIKE wildcards so a query containing % or _ matches those characters literally
    /// instead of silently over-matching. Pairs with ESCAPE '\' in the LIKE fallbacks; the
    /// backslash itself has to be escaped first or it would consume the following character.
    /// </summary>
    ...
    private static byte[] SerializeVector(float[] vector)
    {
        ...
        // Same guard the write path enforces (BertVectorDatabase.StoreVector). Without it a
        // wrong-dimension query embedding reached sqlite-vec as a raw blob and came back as a
        // cryptic MATCH error from the extension instead of naming the actual problem.
        if (vector.Length != BertSearchSchema.EmbeddingDimension)
            throw new ArgumentException(
                $"Expected {BertSearchSchema.EmbeddingDimension} embedding dimensions, got {vector.Length}.",
                nameof(vector));
