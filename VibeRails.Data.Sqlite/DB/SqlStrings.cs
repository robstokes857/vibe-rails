namespace VibeRails.DB
{
    ...
    {
        // Pragmas. journal_mode/busy_timeout are owned by SqliteConnectionFactory and the
        // migration runner; only the WAL size limit is still declared as schema SQL.
        ...
        // LLM) table constraint to be case-insensitive. Lives in MigrationStatements after
        // the legacy schema is adopted. Conflicting names fail the migration transaction
        // without recording completion; names must be corrected before retrying.
        ...
        //
        // UserInputs_fts itself is NOT created here. State migration 2 owns it: it is an
        // external-content index over UserInputSearchDocuments (the exact filtered text), not over
        // UserInputs (raw text), because deleting an ETL-skipped or normalized prompt through the
        // old shape could corrupt the inverted index. Creating the old table here only to have
        // state/2 drop it was wasted work on every fresh database.
        //
        // These two drops stay: legacy auto-insert/auto-update triggers from earlier installs
        // copied raw InputText straight into the index, and if one survived it would write raw
        // text into the rebuilt index behind the content table's back.
